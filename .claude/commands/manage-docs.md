...
| :--- | :--- | :--- |
| Foundation | `docs/01_Foundation/` | 기획, 비전, UI 디자인, 로드맵 |
| Prototype | `docs/02_Prototype/` | UI 프로토타입 리뷰, 화면 흐름 |
| Specs | `docs/03_Specs/` | DB 스키마, API 명세, 구현 가이드 |
| Logic | `docs/04_Logic/` | 비즈니스 로직, 상태 관리, 알고리즘 |
| Test | `docs/05_Test/` | 테스트 시나리오, QA 체크리스트 |

## 레이어 판별

- 기획/UI 디자인/로드맵 → 01_Foundation
- UI 프로토타입 결과/리뷰 → 02_Prototype
- 상세 구현 명세 (DB, API) → 03_Specs
- 비즈니스 로직/상태 머신/알고리즘 → 04_Logic
- 테스트 계획/QA 보고 → 05_Test
...
- 다른 레이어: `../01_Foundation/03_PRODUCT_SPECS.md`
- 특정 섹션 참조 시 Section 번호 명시

### 레이어별 필수 링크

- **Foundation**: 같은 Foundation 문서들 + 관련 Prototype/Specs (있을 경우)
- **Prototype**: Foundation (Product Specs, UI Design) + 이전/다음 Prototype + Specs/Logic (있을 경우)
- **Specs**: Foundation + Prototype + 다른 Specs (DB ↔ API ↔ Storage) + Logic (있을 경우)
- **Logic**: Foundation + Prototype + Specs (DB, API) + Test (있을 경우)
- **Test**: 모든 상위 레이어 참조
