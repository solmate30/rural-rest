...
## 6. 온체인 완성도 (Updated 2026-03-30)

RWA 발행 로직 완성 및 localnet E2E 테스트 완료 후 재점검 결과:
...
- Anchor 프로그램: 9개 instruction 구현, 34개 테스트 케이스 통과 (시나리오 A~E)
- E2E 스크립트: `web/scripts/` 에 00_reset ~ 99_check_state 까지 12개 스크립트로 전체 플로우 자동화
...
2. **emit! 이벤트 없음**: Anchor 프로그램에 이벤트 emit이 없어 Explorer/indexer에서 instruction 해석 불가.
3. **Pyth Oracle 미연동**: `KRW_PER_USDC = 1350` 하드코딩. 실시간 환율 반영 불가.
...
1. **Anchor 프로그램 완성도 높음** — 9개 instruction 전부 구현 (initialize, open_position, purchase, release, cancel, refund, activate, distribute, claim). 34개 테스트 케이스 통과. PDA 구조 정확, 고정소수점 배당 수학 (1e12 precision) 올바름. 단순 데모용이 아닌 실사용 가능한 수준.
...
5. **풀스택 완성도** — Anchor + React Router 7 + Turso + 지갑 연결 + KYC + E2E 스크립트 12개. 개념 증명 수준을 넘음.
...
2. **실제 사용자/부동산 없음** — 개념 증명. 경쟁 프로젝트 (Genesis)는 d-reader라는 실제 플랫폼과 연계. 우리는 갱릉 데이터 리서치 문서는 있지만 실제 부동산이 없음.
3. **Generic 포지셔닝 위험** — 제출 시 "한국 농촌 폐가"를 충분히 강조하지 않으면 crowdedness 204인 일반 부동산 토큰화로 분류됨.
4. **Anchor 이벤트 미구현** — emit! 없어서 indexer/Explorer에서 instruction 의미 파악 불가. 기술 심사 시 감점 요인.
5. **Pyth Oracle 미연동** — KRW/USDC 환율이 1350으로 하드코딩. "프로그래머블 yield"를 강조하면서 환율은 수동이라는 모순.
...
### 냉정한 현재 점수 (10점 만점, Updated 2026-03-30)

| 항목 | 이전 | 현재 | 이유 |
|------|------|------|------|
| 기술 완성도 | 6/10 | **8/10** | 9개 instruction 전부 프론트 연동 완료, 34개 테스트, E2E 스크립트 12개 |
| 혁신성 | 8/10 | 8/10 | 3자 정산, 한국 농촌 폐가 = 독특 (변동 없음) |
| 데모 가능성 | 4/10 | **7/10** | 투자 → 정산 → 수령 전체 플로우 데모 가능. 환불/취소도 동작 |
| 시장 규모/타당성 | 7/10 | 7/10 | 폐가 시장 실재, 글로벌 관광객 수요 증명 가능 (변동 없음) |
| 팀/실행력 | 미정 | 미정 | 문서화 수준 높음, E2E 스크립트 체계적 |

**현재 상태: RWAs 트랙 Honorable Mention 이상 충분히 가능.** devnet 데모에서 투자-정산-수령 전체 플로우를 Explorer 링크로 증명 가능.
**Prize 수상 조건**: devnet 배포 + 데모 영상 + Proof-of-Rental 개념 설계 문서.
...
- `docs/01_Concept_Design/20_IMPLEMENTATION_STAGES.md` — 구현 단계 로드맵 (Stage 1 완료 현황)
