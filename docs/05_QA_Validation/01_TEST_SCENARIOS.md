...
### 3.3. Booking Flow MVP Tests (2026-02-09 추가)
**시나리오**: 게스트가 `/book/:id` 페이지에서 예약 폼을 작성하고 제출합니다.

**구현 파일**: `web/app/routes/book.tsx`

**인증 테스트**:
- [ ] **TC-BK-001**: 비로그인 상태에서 `/book/1` 접근 시 `/auth`로 리다이렉트
- [ ] **TC-BK-002**: 로그인 후 `/book/1` 접근 시 리스팅 데이터 정상 로딩 (제목, 가격, 이미지, 위치)
- [ ] **TC-BK-003**: 존재하지 않는 리스팅 ID로 접근 시 404 응답

**폼 입력 테스트**:
- [ ] **TC-BK-004**: 체크인 날짜 선택 시 오늘 이전 날짜 선택 불가 (`min` 속성)
- [ ] **TC-BK-005**: 체크인 변경 시 체크아웃이 체크인 이전이면 체크아웃 자동 초기화
- [ ] **TC-BK-006**: 인원 선택 드롭다운에 1 ~ maxGuests 범위만 표시
- [ ] **TC-BK-007**: 날짜/인원 변경 시 우측 Summary Card의 가격이 실시간 업데이트

**검증 에러 테스트**:
- [ ] **TC-BK-008**: 필수 필드 미입력 시 "모든 필드를 입력해 주세요." 인라인 에러 표시
- [ ] **TC-BK-009**: 체크아웃 <= 체크인 시 "체크아웃은 체크인 이후여야 합니다." 에러 표시
- [ ] **TC-BK-010**: 과거 날짜 체크인 시 "과거 날짜는 선택할 수 없습니다." 에러 표시
- [ ] **TC-BK-011**: maxGuests 초과 시 "최대 N명까지 예약 가능합니다." 에러 표시

**성공 플로우 테스트**:
- [ ] **TC-BK-012**: 유효한 폼 제출 시 Confirmation UI로 전환 (같은 페이지 내)
- [ ] **TC-BK-013**: Confirmation UI에 예약번호, 체크인/아웃, 인원, 총 금액 정상 표시
- [ ] **TC-BK-014**: "홈으로 돌아가기" 버튼 클릭 시 `/`로 이동
- [ ] **TC-BK-015**: "숙소 다시 보기" 버튼 클릭 시 `/property/:id`로 이동

**Transport Concierge 테스트**:
- [ ] **TC-BK-016**: 픽업 포인트가 있는 숙소에서 Transport Concierge 섹션 표시
- [ ] **TC-BK-017**: 무료 셔틀 배너 및 픽업 포인트 목록 정상 렌더링

**반응형 테스트**:
- [ ] **TC-BK-018**: 모바일 (< 1024px)에서 1열 레이아웃으로 전환
- [ ] **TC-BK-019**: 데스크탑에서 좌측 폼 / 우측 Sticky Summary 2열 레이아웃

**검증 포인트**:
- 에러 메시지가 `bg-red-50 text-red-600 rounded-xl` 스타일로 인라인 표시
- Submit 버튼이 제출 중 "처리 중..." 텍스트로 변경되고 비활성화됨
- Summary Card가 스크롤 시 `sticky top-24`로 고정됨
...
- [ ] **TC-P-003**: 확정 예약 취소 환불율 — 체크인 7일 이상 전 100%, 3~7일 전 50%, 3일 미만 0% (`docs/04_Logic_Progress/15_REFUND_AND_TREASURY_POLICY.md` 참조)
