...
import type { Route } from "./+types/my.portfolio";
...
export async function loader({ request }: Route.LoaderArgs) {
    ...
    if (!session) {
        throw new Response(null, { status: 302, headers: { Location: "/auth?return=/my/portfolio" } });
    }
    ...
    if (!dbUser?.kycVerified || !dbUser.walletAddress) {
        throw new Response(null, { status: 302, headers: { Location: "/kyc?return=/my/portfolio" } });
    }
    ...
    for (const row of investmentRows) {
        ...
        if (onchain) row.tokenStatus = onchain.status;
        if (row.tokenMint && row.tokenStatus === "funding" && row.fundingDeadline) {
            const deadlineMs = row.fundingDeadline instanceof Date
                ? row.fundingDeadline.getTime()
                : Number(row.fundingDeadline) * 1000;
            if (now > deadlineMs) {
                const progressBps = (row.totalSupply ?? 0) > 0
                    ? ((onchain?.tokensSold ?? 0) / (row.totalSupply ?? 1)) * 10000
                    : 0;
                if (progressBps < (row.minFundingBps ?? 6000)) row.tokenStatus = "failed";
            }
        }
    }
    ...
    return {
        portfolioSummary: {
            ...
            currentValue: Math.round(ownedTokens.reduce((s, t) => s + t.totalValue, 0) * 100) / 100,
    ...
    return (
        <div>
            <header className="mb-8">
                <h1 className="text-xl font-bold text-[#4a3b2c] mb-1">{t("portfolio.title")}</h1>
                <p className="text-sm text-stone-500">{t("portfolio.subtitle")}</p>
            </header>

            {!walletParam ? (
                <div className="py-20 text-center">
                    <span className="material-symbols-outlined text-[48px] text-stone-300">account_balance_wallet</span>
                    <p className="text-stone-500 mt-4 mb-4 font-medium">포트폴리오를 불러오는 중입니다</p>
                ...
                <>
                    ...
                    <section className="mb-10">
                        <div className="flex items-baseline justify-between mb-4">
                            <div>
                                <h2 className="text-lg font-bold text-[#4a3b2c] mb-0.5">{t("portfolio.holdings")}</h2>
                                <p className="text-xs text-stone-500">{t("portfolio.holdingsSubtitle")}</p>
                            </div>
                            <span className="text-xs text-stone-400">{t("portfolio.assets", { count: ownedTokens.length })}</span>
                        </div>
