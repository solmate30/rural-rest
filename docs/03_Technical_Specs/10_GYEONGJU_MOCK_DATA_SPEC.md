...
본 문서는 **개발자가 `web/scripts/seed.ts` 파일을 작성·수정할 때 사용하는 최종 확정 명세서**입니다.

> **[2026-03-24 업데이트]** 데이터 소스가 `web/app/data/listings.ts` (정적 Mock) → `web/scripts/seed.ts` (DB seed)로 전환됐습니다.
> 호스트 구조도 단일 운영팀 → **SPV 법인 5개 + 마을 운영자 5명**으로 확정됐습니다. 상세: [17_SPV_STRUCTURE_PLAN.md](../01_Concept_Design/17_SPV_STRUCTURE_PLAN.md)

**목적:** 경주 파일럿 5채의 seed 데이터를 DB에 삽입하기 위한 명확한 수치와 구조 제공
...
| `hostName` | string | 호스트명 (SPV 법인명) | "SPV-3000 주식회사 (황오동 청송재)" |
...
### 2.2. 호스트 (SPV) & 마을 운영자 구성

**구조 원칙:** 1 집 = 1 SPV (법적 주체, `listings.host_id`) = 1 마을 운영자 (실제 운영, `listings.operator_id`)

| listings ID | SPV 법인명 | SPV seed ID | 마을 운영자명 | 운영자 seed ID | 운영자 역할 |
|------------|-----------|------------|------------|--------------|-----------|
| gyeongju-3000 | SPV-3000 주식회사 (황오동 청송재) | seed-spv-3000-hwango | 김황오 | seed-op-3000-hwango | 체크인·청소·황남빵 체험 진행 |
| gyeongju-3001 | SPV-3001 주식회사 (성건동 충재댁) | seed-spv-3001-seonggon | 이성건 | seed-op-3001-seonggon | 다도 체험·한복 대여·조식 준비 |
| gyeongju-3002 | SPV-3002 주식회사 (동천동 신라숲) | seed-spv-3002-dongcheon | 박동천 | seed-op-3002-dongcheon | 자전거 대여·공용 주방 관리·불멍 운영 |
| gyeongju-3003 | SPV-3003 주식회사 (건천읍 월성) | seed-spv-3003-geoncheon | 최건천 | seed-op-3003-geoncheon | 텃밭 체험·작업실 관리·장 담그기 진행 |
| gyeongju-3004 | SPV-3004 주식회사 (안강읍 석굴재) | seed-spv-3004-angang | 정안강 | seed-op-3004-angang | 농작물 수확·조식 준비·바베큐 운영 |

**정산 구조 (영업이익 기준, 전부 온체인 USDC):**
- 40% → 지자체
- 30% → 마을 운영자 (`operator_settlements` 테이블, `claim_tx`로 수령 확인)
- 30% → SPV 주주 배당 (`rwa_dividends` 테이블)

### 2.3. 위치 정보 (GPS 좌표 - 지도 UI 테스트 가능하도록 간격 조정)
...
### 6.1. 완료 (2026-03-24)
- [x] `web/scripts/seed.ts` 작성 — SPV 5개 + 운영자 5명 + listings 5채 + rwaTokens 5개
- [x] `user.role`에 `operator` 추가 (`web/app/db/schema.ts`)
- [x] `listings.operator_id` 컬럼 추가
- [x] `operator_settlements` 테이블 추가 (온체인 USDC 정산)
- [x] `package.json`에 `"seed": "tsx scripts/seed.ts"` 스크립트 추가

### 6.2. seed 실행 방법
```bash
cd web
npm install   # tsx 설치 (최초 1회)
npm run seed
```

### 6.3. 이미지 준비 (미완)
- [ ] Placeholder 이미지 추가 (`/public/images/placeholder-hanok.jpg`)
- [ ] Phase 2: AI 생성 이미지 (5채 × 3장)
- [ ] Phase 3: 실제 현장 촬영 이미지로 교체
...
- [17_SPV_STRUCTURE_PLAN.md](../01_Concept_Design/17_SPV_STRUCTURE_PLAN.md) - SPV 법인 구조 및 수익 분배 설계
- [01_DB_SCHEMA.md](./01_DB_SCHEMA.md) - DB 스키마 (operator_settlements 2.12절)
- `web/scripts/seed.ts` - 실제 seed 코드
