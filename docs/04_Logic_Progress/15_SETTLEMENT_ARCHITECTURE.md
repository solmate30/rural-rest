...
## 6. devnet vs mainnet 차이

| 항목 | devnet (현재) | mainnet (향후) |
|------|------|------|
| 지자체 push tx | `gov_payout_${timestamp}_${random}` (시뮬레이션) | 실제 SPL Token transfer |
| 운영자 push tx | `op_payout_${timestamp}_${random}` (시뮬레이션) | 실제 SPL Token transfer |
| 투자자 claim | Anchor `claimDividend` on localhost:8899 | mainnet Anchor 호출 |
| USDC vault 잔액 | 테스트 USDC | 실제 USDC |
