...
*   **role**: Text ('guest', 'host', 'operator', 'admin') [Default: 'guest'] - 추가 필드 (Better Auth `additionalFields`)
    - `host`: SPV 법인 계정 (법적 운영권 보유, RWA 토큰 발행 주체)
    - `operator`: 마을 운영자 계정 (실제 청소·체크인·유지보수 담당, 영업이익 30% 정산 수령)
...
*   **id**: Text (UUID v4) [Primary Key]
*   **host_id**: Text [Foreign Key -> users.id] — SPV 법인 계정 (법적 주체)
*   **operator_id**: Text [Foreign Key -> users.id, Nullable] — 마을 운영자 계정 (게스트 대면, 정산 수령)
*   **title**: Text [Not Null]
...
### 2.12. `operator_settlements` Table
마을 운영자 월별 정산 내역. 영업이익의 30%를 운영자에게 온체인 USDC로 지급한다.

수익 분배 구조 (매물별 영업이익 기준, 전부 온체인):
- 40% → 지자체 (운영권 제공 대가, 온체인 USDC 송금)
- **30% → 마을 운영자** (청소·체크인·유지보수 노무 대가, 본 테이블에서 관리)
- 30% → SPV 주주 배당 (투자 수익, `rwa_dividends` 테이블에서 관리)

*   **id**: Text (UUID v4) [Primary Key]
*   **operator_id**: Text [Foreign Key -> users.id, Not Null]
*   **listing_id**: Text [Foreign Key -> listings.id, Not Null]
*   **month**: Text [Not Null] — "2026-03" 형식
*   **gross_revenue_krw**: Integer [Not Null] — 해당 월 숙박 매출 합계 (KRW)
*   **operating_cost_krw**: Integer [Default: 0] — 운영비 (청소, 공과금, 소모품 등)
*   **operating_profit_krw**: Integer [Not Null] — 영업이익 = 매출 - 운영비
